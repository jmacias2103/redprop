    ...
    <main style={{ fontFamily: 'sans-serif', background: '#060606', minHeight: '100vh', color: '#ffffff' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2rem', borderBottom: '0.5px solid #222' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          ...
          <button style={{ background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 18px', fontSize: '14px', cursor: 'pointer' }}>Empezar gratis</button>
        </div>
      </nav>

      {/* HERO */}
      <section style={{ textAlign: 'center', padding: '5rem 2rem 3rem' }}>
        <span style={{ background: '#2563eb22', color: '#2563eb', fontSize: '12px', padding: '4px 14px', borderRadius: '99px', marginBottom: '1rem', display: 'inline-block' }}>Para agentes compradores de Argentina</span>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <button style={{ background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px 28px', fontSize: '15px', cursor: 'pointer' }}>Empezar gratis</button>
          <button style={{ background: 'transparent', color: '#2563eb', border: '1.5px solid #2563eb', borderRadius: '8px', padding: '12px 28px', fontSize: '15px', cursor: 'pointer' }}>Ver demo</button>
        </div>
      </section>
      ...
      <section id="planes" style={{ padding: '3rem 2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', maxWidth: '800px', margin: '0 auto' }}>
          {[
            ...
            <div key={i} style={{ background: '#111', border: p.featured ? '2px solid #2563eb' : '0.5px solid #222', borderRadius: '12px', padding: '1.5rem', position: 'relative' }}>
              {p.featured && <span style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: '#2563eb', color: '#fff', fontSize: '11px', padding: '3px 12px', borderRadius: '99px' }}>Más elegido</span>}
              ...
              <button style={{ width: '100%', background: p.featured ? '#2563eb' : 'transparent', color: p.featured ? '#fff' : '#2563eb', border: p.featured ? 'none' : '1.5px solid #2563eb', borderRadius: '8px', padding: '10px', fontSize: '14px', cursor: 'pointer' }}>
      ...
      <footer style={{ padding: '1.5rem 2rem', borderTop: '0.5px solid #222', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: '16px', fontWeight: 500 }}>Red<span style={{ color: '#2563eb' }}>Prop</span></div>
